// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using MyBudget.Api.Infrastructure;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace MyBudget.Api.Infrastructure.Migrations
{
    [DbContext(typeof(BudgetDbContext))]
    [Migration("20260515120000_ActualEntryAttachments")]
    partial class ActualEntryAttachments
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.12")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.Account", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("BaselineId")
                        .HasColumnType("uuid")
                        .HasColumnName("baseline_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<decimal>("InitialBalance")
                        .HasColumnType("numeric")
                        .HasColumnName("initial_balance");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("name");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer")
                        .HasColumnName("sort_order");

                    b.Property<string>("TypeLabel")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("type_label");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_accounts");

                    b.HasIndex("BaselineId")
                        .HasDatabaseName("ix_accounts_baseline_id");

                    b.HasIndex("UserId")
                        .HasDatabaseName("ix_accounts_user_id");

                    b.HasIndex("BaselineId", "Name")
                        .IsUnique()
                        .HasDatabaseName("ix_accounts_baseline_id_name");

                    b.ToTable("accounts", (string)null);
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.ActualEntry", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("AccountId")
                        .HasColumnType("uuid")
                        .HasColumnName("account_id");

                    b.Property<string>("AttachmentBlobKey")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("attachment_blob_key");

                    b.Property<string>("AttachmentContentType")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("attachment_content_type");

                    b.Property<string>("AttachmentOriginalFileName")
                        .HasMaxLength(260)
                        .HasColumnType("character varying(260)")
                        .HasColumnName("attachment_original_file_name");

                    b.Property<decimal>("Amount")
                        .HasColumnType("numeric")
                        .HasColumnName("amount");

                    b.Property<DateOnly>("BookedOn")
                        .HasColumnType("date")
                        .HasColumnName("booked_on");

                    b.Property<Guid>("BudgetPositionId")
                        .HasColumnType("uuid")
                        .HasColumnName("budget_position_id");

                    b.Property<string>("ExternalRef")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("external_ref");

                    b.Property<string>("Note")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("note");

                    b.HasKey("Id")
                        .HasName("pk_actual_entries");

                    b.HasIndex("AccountId")
                        .HasDatabaseName("ix_actual_entries_account_id");

                    b.HasIndex("BudgetPositionId")
                        .HasDatabaseName("ix_actual_entries_budget_position_id");

                    b.ToTable("actual_entries", (string)null);
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.AppUser", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("DisplayName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("display_name");

                    b.HasKey("Id")
                        .HasName("pk_users");

                    b.ToTable("users", (string)null);
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.BaselineInvitation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("AcceptedByUserId")
                        .HasColumnType("uuid")
                        .HasColumnName("accepted_by_user_id");

                    b.Property<Guid>("BaselineId")
                        .HasColumnType("uuid")
                        .HasColumnName("baseline_id");

                    b.Property<DateTimeOffset?>("ConsumedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("consumed_at");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CreatedByUserId")
                        .HasColumnType("uuid")
                        .HasColumnName("created_by_user_id");

                    b.Property<DateTimeOffset>("ExpiresAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expires_at");

                    b.Property<DateTimeOffset?>("RevokedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("revoked_at");

                    b.Property<int>("Role")
                        .HasColumnType("integer")
                        .HasColumnName("role");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("token_hash");

                    b.HasKey("Id")
                        .HasName("pk_baseline_invitations");

                    b.HasIndex("AcceptedByUserId")
                        .HasDatabaseName("ix_baseline_invitations_accepted_by_user_id");

                    b.HasIndex("BaselineId")
                        .HasDatabaseName("ix_baseline_invitations_baseline_id");

                    b.HasIndex("CreatedByUserId")
                        .HasDatabaseName("ix_baseline_invitations_created_by_user_id");

                    b.HasIndex("TokenHash")
                        .IsUnique()
                        .HasDatabaseName("ix_baseline_invitations_token_hash");

                    b.ToTable("baseline_invitations", (string)null);
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.BaselineMember", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("BaselineId")
                        .HasColumnType("uuid")
                        .HasColumnName("baseline_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<int>("Role")
                        .HasColumnType("integer")
                        .HasColumnName("role");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_baseline_members");

                    b.HasIndex("UserId")
                        .HasDatabaseName("ix_baseline_members_user_id");

                    b.HasIndex("BaselineId", "UserId")
                        .IsUnique()
                        .HasDatabaseName("ix_baseline_members_baseline_id_user_id");

                    b.ToTable("baseline_members", (string)null);
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.BudgetBaseline", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid?>("ForkedFromBaselineId")
                        .HasColumnType("uuid")
                        .HasColumnName("forked_from_baseline_id");

                    b.Property<bool>("IsPrimaryBudget")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_primary_budget");

                    b.Property<bool>("IsSampleDemo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_sample_demo");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("name");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("status");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_baselines");

                    b.HasIndex("ForkedFromBaselineId")
                        .HasDatabaseName("ix_baselines_forked_from_baseline_id");

                    b.HasIndex("UserId")
                        .IsUnique()
                        .HasDatabaseName("ux_baselines_user_id_primary_budget")
                        .HasFilter("is_primary_budget IS TRUE");

                    b.ToTable("baselines", (string)null);
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.BudgetPosition", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("BaselineId")
                        .HasColumnType("uuid")
                        .HasColumnName("baseline_id");

                    b.Property<int>("Cadence")
                        .HasColumnType("integer")
                        .HasColumnName("cadence");

                    b.Property<Guid>("CategoryId")
                        .HasColumnType("uuid")
                        .HasColumnName("category_id");

                    b.Property<decimal>("DefaultAmount")
                        .HasColumnType("numeric")
                        .HasColumnName("default_amount");

                    b.Property<DateOnly?>("EndDate")
                        .HasColumnType("date")
                        .HasColumnName("end_date");

                    b.Property<Guid?>("ForkedFromPositionId")
                        .HasColumnType("uuid")
                        .HasColumnName("forked_from_position_id");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("name");

                    b.Property<string>("RecurrenceRuleJson")
                        .HasColumnType("jsonb")
                        .HasColumnName("recurrence_rule_json");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer")
                        .HasColumnName("sort_order");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date")
                        .HasColumnName("start_date");

                    b.HasKey("Id")
                        .HasName("pk_positions");

                    b.HasIndex("BaselineId")
                        .HasDatabaseName("ix_positions_baseline_id");

                    b.HasIndex("CategoryId")
                        .HasDatabaseName("ix_positions_category_id");

                    b.HasIndex("ForkedFromPositionId")
                        .HasDatabaseName("ix_positions_forked_from_position_id");

                    b.ToTable("positions", (string)null);
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.Category", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Color")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("color");

                    b.Property<bool>("IsIncome")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_income");

                    b.Property<bool>("IsSystem")
                        .HasColumnType("boolean")
                        .HasColumnName("is_system");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer")
                        .HasColumnName("sort_order");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_categories");

                    b.HasIndex("UserId", "Name")
                        .IsUnique()
                        .HasDatabaseName("ix_categories_user_id_name");

                    b.ToTable("categories", (string)null);
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.PlannedAmount", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<decimal>("Amount")
                        .HasColumnType("numeric")
                        .HasColumnName("amount");

                    b.Property<Guid>("BudgetPositionId")
                        .HasColumnType("uuid")
                        .HasColumnName("budget_position_id");

                    b.Property<bool>("IsOverride")
                        .HasColumnType("boolean")
                        .HasColumnName("is_override");

                    b.Property<int>("Month")
                        .HasColumnType("integer")
                        .HasColumnName("month");

                    b.Property<int>("Year")
                        .HasColumnType("integer")
                        .HasColumnName("year");

                    b.HasKey("Id")
                        .HasName("pk_planned_amounts");

                    b.HasIndex("BudgetPositionId", "Year", "Month")
                        .IsUnique()
                        .HasDatabaseName("ix_planned_amounts_budget_position_id_year_month");

                    b.ToTable("planned_amounts", (string)null);
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.Account", b =>
                {
                    b.HasOne("MyBudget.Api.Domain.Entities.BudgetBaseline", "Baseline")
                        .WithMany("Accounts")
                        .HasForeignKey("BaselineId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_accounts_baselines_baseline_id");

                    b.HasOne("MyBudget.Api.Domain.Entities.AppUser", "User")
                        .WithMany("Accounts")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_accounts_users_user_id");

                    b.Navigation("Baseline");

                    b.Navigation("User");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.ActualEntry", b =>
                {
                    b.HasOne("MyBudget.Api.Domain.Entities.Account", "Account")
                        .WithMany("ActualEntries")
                        .HasForeignKey("AccountId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_actual_entries_accounts_account_id");

                    b.HasOne("MyBudget.Api.Domain.Entities.BudgetPosition", "BudgetPosition")
                        .WithMany("ActualEntries")
                        .HasForeignKey("BudgetPositionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_actual_entries_positions_budget_position_id");

                    b.Navigation("Account");

                    b.Navigation("BudgetPosition");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.BaselineInvitation", b =>
                {
                    b.HasOne("MyBudget.Api.Domain.Entities.AppUser", "AcceptedByUser")
                        .WithMany("AcceptedInvitations")
                        .HasForeignKey("AcceptedByUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_baseline_invitations_users_accepted_by_user_id");

                    b.HasOne("MyBudget.Api.Domain.Entities.BudgetBaseline", "Baseline")
                        .WithMany("Invitations")
                        .HasForeignKey("BaselineId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_baseline_invitations_baselines_baseline_id");

                    b.HasOne("MyBudget.Api.Domain.Entities.AppUser", "CreatedByUser")
                        .WithMany("CreatedInvitations")
                        .HasForeignKey("CreatedByUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_baseline_invitations_users_created_by_user_id");

                    b.Navigation("AcceptedByUser");

                    b.Navigation("Baseline");

                    b.Navigation("CreatedByUser");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.BaselineMember", b =>
                {
                    b.HasOne("MyBudget.Api.Domain.Entities.BudgetBaseline", "Baseline")
                        .WithMany("Members")
                        .HasForeignKey("BaselineId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_baseline_members_baselines_baseline_id");

                    b.HasOne("MyBudget.Api.Domain.Entities.AppUser", "User")
                        .WithMany("BaselineMemberships")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_baseline_members_users_user_id");

                    b.Navigation("Baseline");

                    b.Navigation("User");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.BudgetBaseline", b =>
                {
                    b.HasOne("MyBudget.Api.Domain.Entities.BudgetBaseline", "ForkedFromBaseline")
                        .WithMany("Forks")
                        .HasForeignKey("ForkedFromBaselineId")
                        .HasConstraintName("fk_baselines_baselines_forked_from_baseline_id");

                    b.HasOne("MyBudget.Api.Domain.Entities.AppUser", "User")
                        .WithMany("Baselines")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_baselines_users_user_id");

                    b.Navigation("ForkedFromBaseline");

                    b.Navigation("User");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.BudgetPosition", b =>
                {
                    b.HasOne("MyBudget.Api.Domain.Entities.BudgetBaseline", "Baseline")
                        .WithMany("Positions")
                        .HasForeignKey("BaselineId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_positions_baselines_baseline_id");

                    b.HasOne("MyBudget.Api.Domain.Entities.Category", "Category")
                        .WithMany("BudgetPositions")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_positions_categories_category_id");

                    b.HasOne("MyBudget.Api.Domain.Entities.BudgetPosition", "ForkedFromPosition")
                        .WithMany("Forks")
                        .HasForeignKey("ForkedFromPositionId")
                        .HasConstraintName("fk_positions_positions_forked_from_position_id");

                    b.Navigation("Baseline");

                    b.Navigation("Category");

                    b.Navigation("ForkedFromPosition");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.Category", b =>
                {
                    b.HasOne("MyBudget.Api.Domain.Entities.AppUser", "User")
                        .WithMany("Categories")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_categories_users_user_id");

                    b.Navigation("User");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.PlannedAmount", b =>
                {
                    b.HasOne("MyBudget.Api.Domain.Entities.BudgetPosition", "BudgetPosition")
                        .WithMany("PlannedAmounts")
                        .HasForeignKey("BudgetPositionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_planned_amounts_positions_budget_position_id");

                    b.Navigation("BudgetPosition");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.Account", b =>
                {
                    b.Navigation("ActualEntries");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.AppUser", b =>
                {
                    b.Navigation("AcceptedInvitations");

                    b.Navigation("Accounts");

                    b.Navigation("BaselineMemberships");

                    b.Navigation("Baselines");

                    b.Navigation("Categories");

                    b.Navigation("CreatedInvitations");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.BudgetBaseline", b =>
                {
                    b.Navigation("Accounts");

                    b.Navigation("Forks");

                    b.Navigation("Invitations");

                    b.Navigation("Members");

                    b.Navigation("Positions");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.BudgetPosition", b =>
                {
                    b.Navigation("ActualEntries");

                    b.Navigation("Forks");

                    b.Navigation("PlannedAmounts");
                });

            modelBuilder.Entity("MyBudget.Api.Domain.Entities.Category", b =>
                {
                    b.Navigation("BudgetPositions");
                });
#pragma warning restore 612, 618
        }
    }
}
